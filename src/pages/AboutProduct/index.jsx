...
const fetchUserImg = async ({ queryKey }) => {
...
export default function AboutProduct() {
    ...
    const [currentImg, setCurrentImg] = useState(data?.main_image || data?.detailed_images[0]);
    ...
    return (
        <>
            <div className='flex items-center gap-2 font-semibold text-[#42A358]/30 mt-5 mb-10'>
                <Link to={-1} className="text-[#42A358]/60 hover:text-[#42A358] hover:pr-2 transi ">Home</Link> / <Link to={`/shop`} className="text-black/80 hover:text-[#42A358] hover:pr-2 transi ">Shop</Link> / <p className="text-[#42A358]">{data?.title || 'product name'}</p>
            </div>
            <div className="flex justify-between items-start gap-10">
                <div className='flex items-center justify-between w-[50%] gap-3'>
                    <div>
                        {data?.detailed_images.map((img, i) => {
                            return (
                    ...
                    <div className='w-[70%]'>
                        <Image src={currentImg} className='max-h-[400px] object-contain' alt="greenshop image" />
                    </div>
                </div>
                <div className='w-[50%]'>
                    <div className='flex justify-between items-center border-[#42A358]/40 pb-4 border-b-2'>
                        <div className='flex items-center gap-3 '>
                            <div className='w-[60px] h-[60px] rounded-full overflow-hidden border-2 logo border-[#42A358]'><img className='w-full h-full object-cover' src={userImg?.profile_photo} alt="user image" /></div>
                    ...
                    <div>
                       ...
                       <div className='flex items-center gap-3'>
                        ...
                        <button className='p-2 texl-lg border border-[#42A358] rounded font-thin'><Heart/></button>
                       </div>
                       <div>
            ...
            <div className='flex items-center gap-5 w-full border-b-2 border-[#42A358]/40 mb-5 py-5 mt-10 text-lg'>
                <h3 onClick={()=>{setIsDesOpen(true); setIsRewOpen(false) }} className={`${isDesOpen && 'text-[#42A358]'} cursor-pointer transi`}>Product Description</h3>
                <h3 onClick={()=>{ setIsDesOpen(false); setIsRewOpen(true)}} className={`${isRewOpen && 'text-[#42A358]'} cursor-pointer transi`}>Reviews (0)</h3>
            </div>
